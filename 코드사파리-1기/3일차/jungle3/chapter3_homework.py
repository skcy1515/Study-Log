...
def question1():
    ...
    target_movie = db.movies.find_one({'title':'파이트 클럽'})
    print (f"{target_movie['title']} : {target_movie['released_year']}")
...
def question2():
    ...
    movies = list(db.movies.find({'released_year': 2025}))

    for movie in movies:
        print(f"{movie['title']}: {movie['released_year']}")

# Q3. 영화의 상영시간이 "펄프 픽션"보다 긴 영화들 중 "석양의 무법자"를 제외한 영화들 모두 삭제해주세요.
def question3():
    target_movie = db.movies.find_one({'title':'펄프 픽션'})
    target_runningTime = target_movie['running_time']

    print(f"{target_movie['title']}의 상영시간은 {target_runningTime}분 입니다.")

    # "펄프 픽션"보다 긴 영화들 중 "석양의 무법자"를 제외한 영화들 삭제
    movies = db.movies.find({
        'running_time': {'$gt': target_runningTime},  # 펄프 픽션보다 긴 영화들
        'title': {'$ne': '석양의 무법자'}  # "석양의 무법자" 제외
    })

    # 하나씩 삭제
    for movie in movies:
        print(f"{movie['title']}가 삭제되었습니다.")
        db.movies.delete_one(movie)
